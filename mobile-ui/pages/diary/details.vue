<template>
	<view class="detail-wrap">
		<u-navbar title="详情" :is-back="true" :background="background" titleColor="#fff"></u-navbar>
		<view class="item-head" style="margin-top: 150upx;">
		...
		<view class="swiper-wrap">
			<swiper class="swiper-box" @change="swiperChange">
				<swiper-item v-for="(item, index) in detail.imgList" :key="index" class="swipers flex-center">
					<image :src="item" mode="widthFix" @tap="ViewImage(index, detail.imgList)" class="img"></image>
				</swiper-item>
			</swiper>
		...
		<view class="comment-wrap">
			<view class="scroll-wrap">
				<view class="" v-if="detail.commentListResponseList !==null">
					<view class="top-num padding-tb">共{{detail.commentListResponseList.length}}条评论</view>
				</view>
				<view class="" v-else>
					<view class="top-num padding-tb">共0条评论</view>
				</view>
				<template v-if="detail.commentListResponseList !==null">
					<view class="comment-item margin-bottom-lg" v-for="(item, index) in detail.commentListResponseList" :key="index">
...
<script>
	import {food_comm_detail} from '../../api/food.js'
	import {food_comment} from '@/api/food_comment.js'
	var that;
	export default {
		data() {
			return {
		...
		methods: {
			...
			getFoodDetail(id){
				let params={}
				params.id=id
				food_comm_detail(params).then(res=>{
					console.log(res)
			...
			addCommon() {
				...
				params.comment=this.commentVal;
				food_comment(params).then(res=>{
					uni.showToast({
						title:"评论成功"
					})
				}).then(()=>{
					this.getFoodDetail(this.detail.id)
				})
