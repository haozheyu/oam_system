	...
	<view :class="['card-item',radius ? 'radius' : '']">
		...
		<view class="content" @tap="toDetails(item.id)">
			<view style="display: flex;flex-direction: row;align-items: center;justify-content: space-between;">
				...
				<u-tag  v-show="item.isTop===1" text="置顶" mode="dark" class="tag"
				type="success"></u-tag>
				
			</view>
			
				
			<view class="img-wrap padding-bottom-lg" v-if="item.imgUrl.length == 1">
				<view class="img-box">
					<image v-for="(child, idx) in item.imgUrl" :key="idx" :src="child" mode="widthFix" class="img" @tap.stop @tap="ViewImage(idx, item.imgUrl)"></image>
				</view>
			</view>
			<view class="img-list padding-bottom-lg" v-if="item.imgUrl.length > 1">
				<view class="img-box" v-for="(child, idx) in item.imgUrl" :key="idx">
					<image :src="child" mode="widthFix" class="img" @tap.stop @tap="ViewImage(idx, item.imgUrl)"></image>
				</view>
			</view>
			<view class="bottom-btn padding-bottom-sm">
				...
				<view class="btn-item flex-center" @tap.stop @tap="handleLike(item.id, item.isLike, item.likeCount)">
...
<script>
		import {food_collect} from '../../api/food.js'
	export default {
		props: {
			obj: {
				type: Object
